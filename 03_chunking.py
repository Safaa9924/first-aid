"""
================================================================================
 STAGE 03 · ADAPTIVE SEMANTIC CHUNKING
 First Aid Reference Guide (St. John Ambulance Canada) — RAG Pipeline
================================================================================
Splits the cleaned document into topic-aware chunks (preserving section
headings, with strict min/max word bounds), then attaches source metadata
to every chunk and saves the final chunk table to CSV.
...
def semantic_chunk_markdown(
    ...
    - Handles boundary cases (like the last chunk or oversized merged chunks).
    """
    ...
    for heading, paragraphs in sections:
        ...
        for para in paragraphs:
            ...
            # Oversized single paragraph -> split into sliding windows
            ...
            # Accumulate paragraph if it fits within target_words
            if current_words + len(para_words) <= target_words:
                ...
                # Extract overlap from previous text
                overlap = " ".join(
                ...
                current_pieces.append(para)
                current_words = (
                    len(h_words) + len(overlap.split()) + len(para_words)
                )
    ...
    for chunk in bounded_chunks:
        ...
        if (
            prev_words < min_chunk_words
            and (prev_words + curr_words) <= max_chunk_words
        ):
            final_chunks[-1] = prev_chunk + "\n\n" + chunk

        elif (
            curr_words < min_chunk_words
            and (prev_words + curr_words) <= max_chunk_words
        ):
            final_chunks[-1] = prev_chunk + "\n\n" + chunk
    ...
    if len(final_chunks) > 1:
        ...
        if (
            last_words < min_chunk_words
            and (last_words + prev_words) <= max_chunk_words
        ):
...
def process_chunk_metadata(
    ...
    # 2. Vectorized Structural Parsing (Header Extraction & Level Detection)
    ...
    df["header_level"] = (
        header_extract["h_level"].str.len().astype("Int64").fillna(0)
    )

    # 3. Vectorized Page Number Handling
    ...
    df["avg_word_length"] = (
        (df["char_count"] / df["word_count"].replace(0, np.nan))
        .round(2)
        .fillna(0.0)
    )
    ...
    # 6. Construct Nested Metadata Payload (Vector DB Ready)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("STAGE 03 · ADAPTIVE SEMANTIC CHUNKING")
    print("=" * 60)
    ...
    print(f"\nSaved chunk table -> {CHUNKS_CSV_PATH}")
    print("Done. Next: run 04_vector_representation.py")
